...
class merlot_widget extends WP_Widget_RSS{

	function merlot_widget() {
	
		$widget_ops = array( 'description' => __('Displays MERLOT Content') );
		$this->WP_Widget( 'merlot_widget', __('MERLOT search and display'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		?>
			<script type="text/javascript" language="javascript">
				merlot_call('<?PHP echo $words[0]; ?>','<?PHP echo $instance["number_links"]; ?>');	
			</script>				
			<ul id='merlot_widget'></ul>
		<?PHP
